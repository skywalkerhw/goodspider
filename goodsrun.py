...
def updateRecord(cago_dict):
    ...
    weight = ''
    if cago_dict.get(u'吨位'):
        weight=cago_dict.get(u'吨位').encode('utf8')
    begindate = cago_dict.get(u'发货日期').encode('utf8')
    routetype = cago_dict.get(u'类型').encode('utf8')
    expiredate = cago_dict.get(u'截止日期').encode('utf8')
    startport = cago_dict.get(u'发出港').encode('utf8')
    destport = cago_dict.get(u'到达港').encode('utf8')
    price = cago_dict.get(u'运价').encode('utf8')
    packtype = cago_dict.get(u'包装').encode('utf8')
    remark=cago_dict.get(u'备注').encode('utf8')
    city=cago_dict.get(u'所属省市').encode('utf8')
    ...
    upatesql = "update tbl_goods_price set begindate = '%s',routetype='%s',weight='%s',expiredate='%s',price='%s',\
    startport='%s',destport='%s',companyname = '%s',linkname='%s',linkphone='%s',linkqq='%s',\
    packtype='%s',remark='%s',city='%s' where code='%s'"  \
    %(begindate,routetype,weight,expiredate,price,startport,destport,\
    companyname,linkname,linkphone,linkqq,packtype,remark,city,code)
...
def createRecord(row):
    ...
    '''
    code = row[0].split("=")[1]
    name = row[1].encode("utf8")
    location=row[2].encode("utf8")
    dest=row[3].encode("utf8")
    pubdate=row[5].encode("utf8")
    now =  datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    insertsql = "insert into tbl_goods_price(code,name,location,dest,pubdate,createtime) values('%s','%s','%s','%s','%s','%s')" \
    %(code,name,location,dest,pubdate, now)
    print insertsql
...
def doFinishMaster(reply):
    ...
    if run_status == STATUS_ZERO:
        ...
        elem.setAttribute('value', 'dsq1030')
        
        elem = document.findFirst('#top1_TextBox2')
        elem.setAttribute('value', 'dsq19851030')
...
def checkEnd(code):
    '''
        检查是否达到抓取终点
    '''
    selectsql = "select code from tbl_goods_price where code = '%s' " %(code)
...
cago_url = 'http://wap.cjsyw.com/hy_index.aspx'
